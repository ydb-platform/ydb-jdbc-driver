...
public class GrpcTestInterceptor implements Consumer<ManagedChannelBuilder<?>>, ClientInterceptor {
    ...
    public <ReqT, RespT> ClientCall<ReqT, RespT> interceptCall(MethodDescriptor<ReqT, RespT> method,
        ...
        // ------ TableService --------
        if (method == TableServiceGrpc.getCreateSessionMethod()) {
            StatusCode status = CREATE_SESSION.poll();
            if (status != null && status != StatusCode.SUCCESS) {
                OperationProtos.Operation op = OperationProtos.Operation.newBuilder()
                        .setReady(true)
                        .setStatus(toPb(status))
                        .build();
                RespT resp = (RespT) YdbTable.CreateSessionResponse.newBuilder().setOperation(op).build();
                return new ErrorCall<>(resp);
            }
        }

        if (method == TableServiceGrpc.getExecuteDataQueryMethod()) {
            StatusCode status = EXECUTE_QUERY.poll();
            if (status != null && status != StatusCode.SUCCESS) {
                OperationProtos.Operation op = OperationProtos.Operation.newBuilder()
                        .setReady(true)
                        .setStatus(toPb(status))
                        .build();
                RespT resp = (RespT) YdbTable.ExecuteDataQueryResponse.newBuilder().setOperation(op).build();
                return new ErrorCall<>(resp);
            }
        }

        if (method == TableServiceGrpc.getCommitTransactionMethod()) {
            StatusCode status = COMMIT_TX.poll();
            if (status != null && status != StatusCode.SUCCESS) {
                OperationProtos.Operation op = OperationProtos.Operation.newBuilder()
                        .setReady(true)
                        .setStatus(toPb(status))
                        .build();
                RespT resp = (RespT) YdbTable.CommitTransactionResponse.newBuilder().setOperation(op).build();
                return new ErrorCall<>(resp);
            }
        }
    ...
    private class FailCall<ReqT, RespT> extends ClientCall<ReqT, RespT> {
